...
export interface AddHeadmanToClassForm {
  studentId: number
  classId: number
}
...
const AddHeadman = () => {
  ...
  const onSubmit = async (data: AddHeadmanToClassForm) => {
    data.classId = Number(router.query!.classId)
    const response = await addHeadManToClass(data)
    if (response.success) {
      await router.back()
    }
  }
    ...
    <Pane>
      <Card padding={48}>
        <Pane display="flex" gap={24}>
          <Header>📝</Header>
          <Header maxWidth={600}>Add headman to class</Header>
        </Pane>
        <Form
          fields={FIELDS}
          {...{
